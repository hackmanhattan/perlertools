...
def get_bead():
	...
	while cur_diff < color_threshold*0.2:
		for i in range(repeat_cnt):
			servo.set_pos_delay(position_dict["home"],servo_delay,True)
			motor.run_delay(motor_duration*1.5,motor_delay)
			cur_color = get_cam_color()
			cur_diff = get_difference(cur_color,default_color)
			if cur_diff > color_threshold*0.2:
				break
		repeat_cnt+=1
		if repeat_cnt > 10:
			print("giving up")
...
def get_cam_color():
	...
	while tmp_color is None:
		try:
			tmp_color = cam.get_color(bead_loc_x,bead_loc_y)
		except:
			tmp_color = None
	cur_color = tmp_color
	return tmp_color
def draw_cam_color():
	...
	for idx in range(len(color_wheel_list)):
		cur_color = hex_tuple(color_wheel_list[idx])
...
def init_cam():
	...
	default_color = Color("#241e1e") #hardcode
...
def main():
	...
	red_bin = ColorBin(Color("#304099"),color_threshold,bead_x_cnt)
